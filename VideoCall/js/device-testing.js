...
const deviceFailAttention =
  '1. Se o navegador solicitar, selecione "Permitir"<br>' +
  '2. Se o software antivírus exibir um prompt, selecione "Permitir"<br>' +
  '3. Verifique as configurações do sistema para permitir o acesso do navegador à câmera e ao microfone<br>' +
  '4. Verifique as configurações do seu navegador para permitir que páginas da web acessem sua câmera e microfone<br>' +
  '5. Verifique se a câmera/microfone está corretamente conectada e ligada<br>' +
  '6. Tente reconectar a câmera/microfone<br>' +
  '7. Tente reiniciar o dispositivo e detectá-lo novamente.';
const networkFailAttention =
  '1. Verifique se o dispositivo está conectado à Internet<br>' + '2. Atualize a página da web e verifique novamente<br>' + '3. Por favor, tente mudar a rede e testar novamente.';
...
function getDeviceConnectInfo() {
  let connectInfo = 'Erro de conexão, tente novamente';
  // 第一步：浏览器未检测到摄像头/麦克风/扬声器设备的提示
  if (!(hasCameraDevice && hasMicDevice && hasVoiceDevice)) {
    connectInfo = `não detectado${hasCameraDevice ? '' : '【Câmera】'}${
      hasVoiceDevice ? '' : '【palestrante】'
    }${hasMicDevice ? '' : '【microfone】'}dispositivo, verifique a conexão do dispositivo`;
    return connectInfo;
  }
  // 第二步：浏览器未拿到摄像头/麦克风权限的提示
  if (!(hasCameraConnect && hasMicConnect)) {
    connectInfo = hasNetworkConnect
      ? 'Permita que o navegador e a página da web acessem o dispositivo de câmera/microfone'
      : 'Permita que o navegador e a página da web acessem a câmera/microfone e verifiquem a conexão de rede';
    // 显示设备连接失败引导
  ...
  if (!hasNetworkConnect) {
    connectInfo = 'Falha na conexão de rede, verifique a conexão de rede';
    // 显示设备连接失败引导
    $('#connect-attention-container').show();
...
async function startDeviceConnect() {
  ...
  let showDeviceConnectInfo = function() {
    ...
    $('#device-loading').show();
    $('#connect-info')
      .text('O dispositivo está se conectando. Aguarde.')
      .css('color', '#cccccc');
    ...
    setTimeout(() => {
      ...
      if (connectResult) {
        $('#connect-info')
          .text('O dispositivo e a rede foram conectados com sucesso. Inicie a detecção do dispositivo.')
          .css('color', '#32CD32');
        $('#connect-again-btn').hide();
        $('#start-test-btn')
...
async function startNetworkTesting() {
  ...
  $('<div></div>')
    .text(isScreenShareSupported ? 'suporta' : 'não suporta')
  ...
  const intervalId = setInterval(() => {
    if (countDown === 0) {
      ...
      $('#uplink-network')
        ...
        .text([String(networkTestingResult.average.uplinkNetworkQuality)]);
...
function handleGetUserMediaError(error) {
  switch (error.name) {
    case 'NotReadableError':
      ...
      alert(
        'A câmera/microfone está temporariamente indisponível. Verifique se o sistema concede permissões de câmera/microfone ao navegador atual e se nenhum outro aplicativo está ocupando a câmera/microfone.'
      );
      return;
    case 'NotAllowedError':
      alert('O usuário/sistema teve acesso autorizado negado à câmera ou microfone');
      return;
    case 'NotFoundError':
      // 找不到摄像头或麦克风设备
      alert('Dispositivo de câmera ou microfone não encontrado');
      return;
    case 'OverConstrainedError':
      alert(
        'As propriedades da coleção serão definidas incorretamente se você especificou cameraId/microphoneId，Por favor, certifique-se de que eles sejam uma string válida e não vazia'
      );
      return;
    default:
      alert('Um erro desconhecido foi encontrado ao inicializar o stream local. Tente novamente');
      return;
  }
}
